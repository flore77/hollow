...
public interface PublishListener extends HollowProducerEventListener {
    ...
    /**
     * Called once a blob has been published successfully or failed to published. Use {@link Status.Publish#getBlob()}
     * to get more details on blob type and size.
     * This method is called for every {@link com.netflix.hollow.api.producer.HollowProducer.Blob.Type} that was published.
     *
     * @param publishStatus Status of publishing. {@link Status.Publish#getType()} returns {@code SUCCESS} or {@code FAIL}.
     * @param elapsed time taken to publish the blob
     */
    void onBlobPublish(Status.Publish publishStatus, Duration elapsed);
     ...
     *
     * @param status CycleStatus of the publish stage. {@link Status.Stage#getType()} will return {@code SUCCESS}
     * when the publish was successful; @{code FAIL} otherwise.
     * @param elapsed duration of the publish stage in {@code unit} units
     */
    void onPublishComplete(Status.Stage status, Duration elapsed);
}
