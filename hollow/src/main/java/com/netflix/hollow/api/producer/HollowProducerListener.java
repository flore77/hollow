...
public interface HollowProducerListener extends
    ...
    @Override
    default void onProducerRestoreComplete(com.netflix.hollow.api.producer.Status.RestoreStage status, Duration elapsed) {
        onProducerRestoreComplete(new RestoreStatus(status), elapsed.toMillis(), TimeUnit.MILLISECONDS);
    }
    ...
    @Override
    default void onCycleComplete(com.netflix.hollow.api.producer.Status.StageWithState status, Duration elapsed) {
        onCycleComplete(new ProducerStatus(status), elapsed.toMillis(), TimeUnit.MILLISECONDS);
    }
    ...
    @Override
    default void onPopulateComplete(com.netflix.hollow.api.producer.Status.Stage status, Duration elapsed) {
        onPopulateComplete(new ProducerStatus(status), elapsed.toMillis(), TimeUnit.MILLISECONDS);
    }
    ...
    @Override
    default void onBlobPublish(com.netflix.hollow.api.producer.Status.Publish publishStatus, Duration elapsed) {
        onArtifactPublish(new PublishStatus(publishStatus), elapsed.toMillis(), TimeUnit.MILLISECONDS);
    }
    ...
    @Override
    default void onPublishComplete(com.netflix.hollow.api.producer.Status.Stage status, Duration elapsed) {
        onPublishComplete(new ProducerStatus(status), elapsed.toMillis(), TimeUnit.MILLISECONDS);
    }
    ...
    @Override
    default void onIntegrityCheckComplete(com.netflix.hollow.api.producer.Status.StageWithState status, Duration elapsed) {
        onIntegrityCheckComplete(new ProducerStatus(status), elapsed.toMillis(), TimeUnit.MILLISECONDS);
    }
    ...
    @Override
    default void onAnnouncementComplete(com.netflix.hollow.api.producer.Status.StageWithState status, Duration elapsed) {
        onAnnouncementComplete(new ProducerStatus(status), elapsed.toMillis(), TimeUnit.MILLISECONDS);
    }
    ...
    class ProducerStatus {
        ...
        ProducerStatus(com.netflix.hollow.api.producer.Status.Stage s) {
        ...
        ProducerStatus(com.netflix.hollow.api.producer.Status.StageWithState s) {
            this.status = s.getType() == com.netflix.hollow.api.producer.Status.Stage.StatusType.SUCCESS
                    ? Status.SUCCESS : Status.FAIL;
            this.version = s.getVersion();
            this.throwable = s.getCause();
            this.readState = s.getReadState();
        }

        ProducerStatus(Status status, long version, HollowProducer.ReadState readState, Throwable throwable) {
            this.status = status;
    ...
    class RestoreStatus {
        ...
        RestoreStatus(com.netflix.hollow.api.producer.Status.RestoreStage s) {
            this.status = Status.of(s.getType());
            this.throwable = s.getCause();
            this.versionDesired = s.getVersionDesired();
            this.versionReached = s.getVersionReached();
        }
    ...
    class PublishStatus {
        ...
        PublishStatus(com.netflix.hollow.api.producer.Status.Publish s) {
            this.status = Status.of(s.getType());
            this.throwable = s.getCause();
            this.blob = s.getBlob();
        }
